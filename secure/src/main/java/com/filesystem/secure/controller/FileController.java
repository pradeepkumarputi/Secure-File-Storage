...
public class FileController {
    ...
    public ResponseEntity<FileResponse> uploadFile(@RequestParam("file") MultipartFile file) {
        try {
            SecureFile savedFile = fileService.uploadFile(file);
...
public ResponseEntity<Resource> downloadFile(@PathVariable Long fileId, @RequestParam("key") String downloadKey) {
    try {
        FileService.FileDownloadDto downloadDto = this.fileService.downloadFile(fileId, downloadKey);
        ...
        return ResponseEntity.status(403).build(); // Forbidden if key is invalid
    } catch (Exception e) {
    ...
    @GetMapping("/{fileId}")
    public ResponseEntity<Resource> getFileById(@PathVariable Long fileId, @RequestParam("key") String downloadKey) {
        try {
            FileService.FileDownloadDto downloadDto = fileService.downloadFile(fileId, downloadKey);
            ByteArrayResource resource = new ByteArrayResource(downloadDto.getContent());
    ...
    public ResponseEntity<Void> deleteFile(@PathVariable Long fileId, @RequestParam("key") String downloadKey) {
        try {
            // Verify the download key first
            String actualKey = fileService.getDownloadKey(fileId);
            if (!actualKey.equals(downloadKey)) {
